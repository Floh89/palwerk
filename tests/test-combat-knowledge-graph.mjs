...
import { COMBAT_GRAPH, DATA_STATUSES, ELEMENT_COUNTER, createCombatKnowledgeGraph, encounterPhases, graphQualitySummary, normalizeInternalId } from '../src/knowledge/combat-graph.js';
import { resolvePalId, resolvePalKnowledge } from '../src/knowledge/pal-ids.js';
...
assert.equal(new Set(COMBAT_GRAPH.pals.map(pal=>pal.id)).size,COMBAT_GRAPH.pals.length,'Technische Pal-IDs müssen eindeutig sein');
...
for(const pal of COMBAT_GRAPH.pals){
  assert.ok(pal.id&&pal.catalogId&&pal.name,'Jeder Pal braucht getrennte technische ID, Katalog-ID und Anzeigename');
...
const orserk=resolvePalKnowledge('orserk');
assert.ok(orserk,'Kanonische ID orserk muss auflösbar sein');
assert.equal(resolvePalId('Orserk'),'orserk');
assert.ok(orserk.elements.includes('Elektro'));
assert.notEqual(orserk.id,resolvePalId('Orserk'),'Technische Quell-ID und kanonische PALWERK-ID müssen getrennt bleiben');
...
assert.equal(new Set(duplicateGraph.pals.map(pal=>pal.id)).size,1,'Doppelte technische IDs müssen in der Quelle erkennbar bleiben');
