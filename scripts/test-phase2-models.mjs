...
test('API-Version ist für Phase 2 erhöht',()=>assert.equal(OPTIMIZER_API_VERSION,'2.0.0'));
test('Normales Team verwendet Single-Active-Modell',()=>{const r=optimizeTeam({activity:'normal_team',encounter:{name:'Test',elements:['Feuer']},ownedPals:ownedCombat,constraints:{ownedOnly:true}});assert.equal(r.model,'single-active-pal');assert.equal(r.status,'ok');assert.equal(r.teams[0].members.length,5);assert.equal(r.teams[0].members.filter(x=>x.estimatedDpsRange).length,1);});
...
test('Passive Partyplätze erhalten keinen eigenen Pal-DPS',()=>{const r=optimizeTeam({activity:'tower',encounter:tower,ownedPals:ownedCombat,constraints:{ownedOnly:true}});for(const m of r.teams[0].members.slice(1)){assert.equal(m.relativeCombatValue,0);assert.equal(m.estimatedDpsRange,null);assert.equal(m.rotation.length,0);}});
...
test('Raid und normales Team besitzen unterschiedliche Ergebnisverträge',()=>{const normal=optimizeTeam({activity:'normal_team',encounter:tower,ownedPals:ownedCombat,constraints:{ownedOnly:true}}),army=optimizeTeam({activity:'raid',encounter:raid,ownedPals:ownedCombat,constraints:{baseLimit:3}});assert.ok(Array.isArray(normal.teams));assert.ok(!('armies'in normal));assert.ok(Array.isArray(army.armies));assert.ok(!('teams'in army));});
