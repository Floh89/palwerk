...
const combatPals=PAL_CATALOG.filter(p=>p?.stats?.attack>0&&(p.skills||[]).some(s=>Number(s?.data?.power??s?.power)>0));
const samplePal=combatPals[0];
...
test('Rotation enthält höchstens drei eindeutige Skills',()=>{const r=buildRotation(samplePal,DEFAULT_PAL_PROFILE,raids[0]);assert.ok(r.length<=3);assert.equal(r.length,new Set(r.map(x=>x.s.id)).size);});
test('Aktive Skill-Auswahl wird respektiert',()=>{const natural=(samplePal.skills||[]).filter(s=>Number(s?.data?.power??s?.power)>0);if(natural.length<2)return;const profile={...DEFAULT_PAL_PROFILE,activeSkillIds:[natural[0].id]};const r=buildRotation(samplePal,profile,raids[0]);assert.ok(r.every(x=>x.s.id===natural[0].id));});
...
test('Bestandsmodus verwendet nur erlaubte Pals',()=>{const owned=PAL_CATALOG.slice(0,3).map(p=>p.key);const r=optimizeRaidArmy({encounter:raids[0],maxSlots:12,ownedOnly:true,ownedIds:owned});assert.ok(r.army.every(x=>owned.includes(x.pal.key)));});
...
test('Gegner mit unterschiedlichen Elementen verändern mindestens die Rotation oder Rangfolge',()=>{const fire=normalTowers.find(x=>x.elements.includes('Feuer'));const dark=normalTowers.find(x=>x.elements.includes('Schatten'));if(!fire||!dark)return;const a=optimizeRaidArmy({encounter:fire,maxSlots:5});const b=optimizeRaidArmy({encounter:dark,maxSlots:5});const sig=x=>x.army.map(m=>`${m.pal.key}:${m.rotation.map(s=>s.name).join('|')}`).join(',');assert.notEqual(sig(a),sig(b));});
